...
지난번에 하신 것처럼 깃허브를 통해 웹에 올려서 다른 사람들에게 이 멋진 홈페이지를 보여주실 수 있습니다! 

1. **파일 업로드**: 깃허브 저장소(`sample_Repository`)에 있는 기존 `app.py` 내용 전체를 지우고, 위 코드를 복사해서 붙여넣은 뒤 저장합니다.
2. **라이브러리 설정**: `requirements.txt`에 `pandas`도 필요하므로 아래 3줄로 내용을 변경해 줍니다.
